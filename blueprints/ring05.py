...
ring_bp = Blueprint('ring05', __name__,url_prefix='/ring05')
...
def generate_image(model_name, prompt_text, batch_size):
    ...
    try:
        ...
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        workflow_path = os.path.join(root_dir, 'workflow/005-ring', model_name)
...
@ring_bp.route('/api/generate-text', methods=['POST'])
def generate_text():
    try:
...
@ring_bp.route('/api/image-to-text', methods=['POST'])
...
@ring_bp.route('/api/create-image', methods=['POST'])
...
@ring_bp.route('/api/get-models', methods=['GET'])
def get_models():
    try:
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        models_folder = os.path.join(root_dir, 'workflow/005-ring')
...
@ring_bp.route('/output/<path:filename>')
