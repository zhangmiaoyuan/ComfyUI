...
necklace_bp = Blueprint('necklace', __name__,url_prefix='/necklace')
...
def generate_image(model_name, prompt_text, batch_size):
    ...
    try:
        ...
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        workflow_path = os.path.join(root_dir, 'workflow/004-necklace', model_name)
...
@necklace_bp.route('/api/generate-text', methods=['POST'])
def generate_text():
    try:
...
@necklace_bp.route('/api/image-to-text', methods=['POST'])
...
@necklace_bp.route('/api/create-image', methods=['POST'])
...
@necklace_bp.route('/api/get-models', methods=['GET'])
def get_models():
    try:
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        models_folder = os.path.join(root_dir, 'workflow/004-necklace')
...
@necklace_bp.route('/static/output/<path:filename>')
def serve_image(filename):
    try:
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        image_path = os.path.join(root_dir, 'static', 'output', filename)
