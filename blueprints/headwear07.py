...
headwear_bp = Blueprint('headwear07', __name__,url_prefix='/headwear07')
...
def generate_image(model_name, prompt_text, batch_size):
    ...
    try:
        ...
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        workflow_path = os.path.join(root_dir, 'workflow/007-headwear', model_name)
...
@headwear_bp.route('/api/generate-text', methods=['POST'])
def generate_text():
    try:
...
@headwear_bp.route('/api/image-to-text', methods=['POST'])
...
@headwear_bp.route('/api/create-image', methods=['POST'])
...
@headwear_bp.route('/api/get-models', methods=['GET'])
def get_models():
    try:
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        models_folder = os.path.join(root_dir, 'workflow/007-headwear')
...
@headwear_bp.route('/output/<path:filename>')
